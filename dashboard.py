...
class dashboard(ctk.CTkToplevel):
    def __init__(self, master:ctk.CTk, entry_key: str, _date_logged: datetime):
        ...
        #divider
        ctk.CTkFrame(self.top_frame, width=width*0.0025, fg_color=Color.Grey_Bright_2,corner_radius=0).grid(row=0, column=2, padx=(width*0.0025,width*0.01),)

        self.acc_btn = cctk.ctkButtonFrame(self.top_frame, width=round(self.top_frame.winfo_reqwidth() * .115),
        ...
        self.acc_btn.grid(row=0, column=3, sticky='e', padx=(width*0.005))
...
class dashboard_frame(ctk.CTkFrame):
    ...
    def __init__(self, master):
        ...
        self.inventory_stat_label.grid(row=0, column=0, sticky="w", padx=(self.inventory_frame_width*0.03,0),pady=(self.inventory_frame_height*0.045,self.inventory_frame_height*0.0125))
        
        self.inventory_content_frame = ctk.CTkFrame(self.inventory_stat_frame, height=self.inventory_frame_height*0.16, fg_color="transparent", corner_radius=0)
        self.inventory_content_frame.grid_propagate()
        self.inventory_content_frame.grid_columnconfigure((0,1), weight=1)
        self.inventory_content_frame.grid(row=1, column=0, sticky="new", padx=self.inventory_frame_width*0.0325, pady=(0,self.inventory_frame_height*0.015))
        
        self.safety_button = dashboard_popup.status_bar(self.inventory_content_frame, info=(), height=height*0.08, width=self.inventory_content_frame._current_width,
                                                        fg_color=Color.Safe_color, hover_color=Color.Hover_Safe_color,
                                                        text="Safe/Normal", count=len(self.safe), icon_color='transparent', window=self.show_status_popup, data=self.safe )
        self.safety_button.grid(row=0, column=0, columnspan=1, sticky="nsew", pady=(height*0.015, width*0.005), padx=(0, width*0.005))
        
        self.near_button = dashboard_popup.status_bar(self.inventory_content_frame, info=(), height=height*0.08, width=self.inventory_content_frame._current_width, 
                                                        fg_color=Color.Near_Expire_Color, hover_color=Color.Hover_Near_Expire_Color,
                                                        text="Near Expire", count=len(self.near_expired), icon_color=Color.Red_Pastel, window=self.show_status_popup, data=self.near_expired )
        self.near_button.grid(row=0, column=1, columnspan=2, pady=(height*0.015, width*0.005), sticky='nsew')
        
        self.reorder_button = dashboard_popup.status_bar(self.inventory_content_frame, info=(), height=height*0.08, width=self.inventory_content_frame._current_width,
                                                         fg_color=Color.Reorder_Color, hover_color=Color.Hover_Reorder_Color,
                                                        text="Reorder", count=len(self.reorder), icon_color=Color.Red_Pastel, window=self.show_status_popup, data=self.reorder )
        self.reorder_button.grid(row=1, column=0, columnspan=1, sticky="nsew", padx=(0, width*0.005), pady=(0, width*0.005))
        
        self.expire_button = dashboard_popup.status_bar(self.inventory_content_frame, info=(), height=height*0.08, width=self.inventory_content_frame._current_width, 
                                                        fg_color=Color.Expire_Color, hover_color=Color.Hover_Expire_Color,
                                                        text="Expired", count=len(self.expired), icon_color=Color.Red_Pastel, window=self.show_status_popup, data=self.expired )
        self.expire_button.grid(row=1, column=1, columnspan=1, sticky='nsew', pady=(0, width*0.005) )
        
        self.critical_button = dashboard_popup.status_bar(self.inventory_content_frame, info=(), height=height*0.08, width=self.inventory_content_frame._current_width,
                                                            fg_color=Color.Critical_Color, hover_color=Color.Hover_Critical_Color,
                                                        text="Critical", count=len(self.critical), icon_color=Color.Red_Pastel, window=self.show_status_popup, data=self.critical )
        self.critical_button.grid(row=2, column=0, columnspan=1, sticky="nsew", padx=(0, width*0.005),pady=(0, width*0.005))
        
        self.out_stock_button = dashboard_popup.status_bar(self.inventory_content_frame, info=(), height=height*0.08, width=self.inventory_content_frame._current_width,
                                                            fg_color=Color.Out_Stock_Color, hover_color=Color.Hover_Out_Stock_Color,
                                                        text="Out of Stock", count=len(self.out_stock), icon_color=Color.Red_Pastel, window=self.show_status_popup, data=self.out_stock )
    ...
    def show_pie(self, data: list = None):
        ...
        ax.pie(data, autopct=f"{'%0.1f%%'if self.data[0] + self.data[1] > 0 else ''}", 
               startangle=0,counterclock=0, explode=(0.1,0), colors=[Color.Light_Green, Color.Red_Tulip] if self.data[0] + self.data[1] > 0 else [Color.White_Platinum],
                textprops={'fontsize':14, 'color': Color.White_Ghost, 'family':'monospace', 'weight':'bold' },)
...
class reception_frame(ctk.CTkFrame):
    def __init__(self, master):
        ...
        '''INVOICE FRAME: START'''
        self.invoice_frame = ctk.CTkFrame(self, fg_color=Color.White_Lotion)
        ...
        self.tab_frame.grid(row=1, column=0, sticky="new" ,padx=(width*0.005), pady=(height*0.005, 0))
        self.tab_frame.grid_columnconfigure(4, weight=1)
        ...
        self.item_tab = cctk.ctkButtonFrame(self.tab_frame, cursor="hand2", height=height*0.055, width=width*0.125,fg_color=Color.White_Color[7], corner_radius=0, hover_color=Color.Blue_LapisLazuli_1, bg_color=selected_color)
        ...
        self.account_tab_icon = ctk.CTkLabel(self.item_tab, text="",image=self.trash_icon)
        self.account_tab_icon.pack(side="left", padx=(width*0.01,width*0.0025))
        self.accounts_label = ctk.CTkLabel(self.item_tab, text="ITEMS", text_color="white",font=('DM Sans Medium', 14))
        ...
        '''INVOICE FRAME ITEMS'''
        self.item_treeview_frame = ctk.CTkFrame(self.invoice_frame)
        self.item_treeview_frame.grid(row=2, column=0, sticky="nsew",padx=(width*0.004), pady=(0,height*0.01))

        self.item_invoice_treeview = cctk.cctkTreeView(self.item_treeview_frame, width= width * .805, height= height * .684, corner_radius=0,
                                           column_format=f'/No:{int(width*.025)}-#r/ReceptionID:{int(width*.115)}-tc/ClientName:x-tl/Total:{int(width*.1)}-tr/Date:{int(width*.1)}-tc!30!30',
                                           double_click_command= self.load_invoice_content)
        self.active = self.item_invoice_treeview
        self.item_invoice_treeview.pack()

        self.proceeed_button = ctk.CTkButton(self.item_treeview_frame, text="Proceed to Payment", image=self.proceed_icon, height=height*0.05, width=width*0.135,font=("Arial", 14),
        ...
        self.scheduling_invoice_treeview = cctk.cctkTreeView(self.scheduling_frame, width= width * .805, height= height * .684, corner_radius=0,
                                                             column_format=f'/No:{int(width*.025)}-#r/ReceptionID:{int(width*.115)}-tc/Pet:x-tl/Owner:x-tl/Service:{int(width*.15 )}-tl/Total:{int(width*.1)}-tr/Date:{int(width*.1)}-tc!30!30',)                                                             #column_format=f'/No:{int(width*.025)}-#r/ReceptionID:{int(width*.115)}-tc/Per:x-tl/Services:x-tl/Date:{int(width*.1)}-tc!30!30')
        self.scheduling_invoice_treeview.pack()
        self.proceeed_button = ctk.CTkButton(self.scheduling_frame, text="Proceed to Payment", image=self.proceed_icon, height=height*0.05, width=width*0.135,font=("Arial", 14),
                                             compound="right", command = self.proceed_to_payment)
        self.proceeed_button.pack(padx = (0, (width*.0025)), anchor = 'e', pady = (height * .005))
...
class sales_frame(ctk.CTkFrame):
    ...
    def __init__(self, master):
        ...
        '''TO'''
        self.to_date_frame = ctk.CTkFrame(self.date_frame, fg_color=Color.White_Platinum, height=height*0.05, width=width*0.15)
        ...
        self.columns = ("rec_no", "or_num","client","total","date","cashier")
        self.column_names = ("No", "OR ","Client", "Total", "Date", "Cashier")
        
        self.sales_tree = ttk.Treeview(self.sales_table_frame, columns=self.columns, show="headings",)
...
class inventory_frame(ctk.CTkFrame):
    ...
    def __init__(self, master):
        ...
        def sort_status_configuration_callback(_: any = None):
            ...
            self.disposal_btns_frame.grid(row=3, column=0, pady=(height*0.01), sticky="nsew", padx=(width*0.005)) if 'Expiry' in self.sort_type_option.get() and 'Expired' in self.sort_status_option.get() else self.disposal_btns_frame.grid_forget()
        ...
        self.restock_btn = ctk.CTkButton(self.inventory_sub_frame, width=width*0.1, height = height*0.055, text="Stock Order", image=self.restock_icon, font=("DM Sans Medium", 14),
        ...
        self.dispose_all_btn = ctk.CTkButton(self.disposal_btns_frame, width=width*0.1, height = height*0.05, text="Dispose All", image=Icons.delete_all_icon, font=("DM Sans Medium", 14),
                                             fg_color=Color.Red_Pastel, hover_color=Color.Red_Hover, command=batch_dispose)
        self.dispose_all_btn.pack(side='left', padx=(0, width*0.005))
        
        self.dispose_btn = ctk.CTkButton(self.disposal_btns_frame, width=width*0.1, height = height*0.05, text="Dispose", image=Icons.delete_one_icon, font=("DM Sans Medium", 14),
                                         fg_color=Color.Red_Pastel, hover_color=Color.Red_Hover)
        #self.dispose_btn.pack(side='left')
        ...
        #self.data1 = database.fetch_data(sql_commands.get_inventory_by_group, None)
        self.data_view1 = cctk.cctkTreeView(self.tree_view_frame, data = [], width= width * .8, height= height * .7, corner_radius=0,
                                           column_format=f'/No:{int(width*.025)}-#r/Rate:{int(width*.045)}-tc/ItemBrand:{int(width*.08)}-tl/ItemDescription:x-tl/StockPcs:{int(width*.085)}-tr/Price:{int(width*.085)}-tr/ExpirationDate:{int(width*.1)}-tc/Status:{int(width*.1)}-tc!30!30',
        ...
        '''def full_dispose_all():
            ...
            for d in data:
                print(d)
                item_id = database.fetch_data("Select item_uid from recieving_item where id = ?", (d[0], ))[0][0]
        ...
        self.rs_add_item_btn.grid(row=0, column=2, sticky="w", padx=(0,width*0.005), pady=(height*0.01))


        self.info_btn = ctk.CTkButton(self.restock_frame, width=height*0.05, height = height*0.05, image=Icons.info_icon, text="", font=("DM Sans Medium", 14),
                                             command=show_order_info,)
        self.info_btn.grid(row=0, column=4, sticky="e", padx=(0,width*0.005), pady=(height*0.01))
        ...
        self.ds_sort_category_option= ctk.CTkOptionMenu(self.ds_sort_frame, values=["Medicine","Food","Accessories"], anchor="center", font=("DM Sans Medium", 12), width=width*0.1, dropdown_fg_color=Color.White_AntiFlash,  fg_color=Color.White_Color[3],
                                                 text_color=Color.Blue_Maastricht, button_color=Color.Blue_Tufts,)
        self.ds_sort_category_option.grid(row=0, column=1, padx=(width*0.005), pady=(width*0.005), sticky="nsew")
        
        self.ds_sort_type_option= ctk.CTkOptionMenu(self.ds_sort_frame, values=["Expired"], anchor="center", font=("DM Sans Medium", 12), width=width*0.1, dropdown_fg_color=Color.White_AntiFlash,  fg_color=Color.White_Color[3],
                                                 text_color=Color.Blue_Maastricht, button_color=Color.Blue_Tufts,)
        self.ds_sort_type_option.grid(row=0, column=3, padx=(0, width*0.005), pady=(width*0.005), sticky="nsew")
        ...
        self.date_frame = ctk.CTkFrame(self.disposal_frame, fg_color=Color.White_Platinum)
        ...
        self.year_option = ctk.CTkOptionMenu(self.date_frame, values=self.operational_year, width=width*0.1, font=("DM Sans Medium",14), 
                                                 height=height*0.045, anchor="center",)
