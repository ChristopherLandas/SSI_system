...
def authorization(master, info:tuple, command_callback :Optional[callable] = None):
    class instance(ctk.CTkFrame):
        def __init__(self, master, info:tuple, command_callback :Optional[callable]):
            ...
            self.upper_frame = ctk.CTkFrame(self, height= height * .13, fg_color= Color.Blue_Yale, corner_radius= 0)
            ...
            self.user_label.grid(row=2, column=0,sticky='sw',pady = (width * .05, 0), padx = 44)
            ...
            self.user_frame.grid(row=3, column=0,sticky='ew', padx=(44,44))
            ...
            self.password_label = ctk.CTkLabel(self.main_frame, text='Password', font=('DM Sans Medium',17),
                                        text_color=Color.Grey_Davy)
            self.password_label.grid(row=4, column=0,sticky='sw',padx=(44,44),pady=(20,0))

            '''Frame for the password entry'''
            self.pass_frame = ctk.CTkFrame(self.main_frame, border_color=Color.Grey_Davy, border_width=2,
                                        fg_color=Color.White_Milk, width = width * .75)
            self.pass_frame.grid(row=5, column=0,sticky='ew',padx= 44)
            ...
            self.login_button.grid(row=7, column=0, sticky='nsew', padx=(42,35),pady=(35,35))
